...
cp /vagrant/config/server2/default /etc/nginx/sites-available/
cp /vagrant/config/server2/my.cnf /etc/mysql/
...
mysql -u root --password="pass1" -e "GRANT REPLICATION SLAVE ON *.* TO 'repl'@'192.168.10.100' IDENTIFIED BY 'slavepass';FLUSH PRIVILEGES;"
mysql -u root --password="pass1" -e "CHANGE MASTER TO MASTER_HOST='192.168.10.100',MASTER_USER='repl',MASTER_PASSWORD='slavepass',MASTER_LOG_FILE='mysql-bin.000001',MASTER_LOG_POS=107;START SLAVE;"
mount -a
echo "1" > /root/block
fi
